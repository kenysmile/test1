...
var websocket = new WebSocket('ws://localhost:8082/SIP');
    ...
    var FieldPhone = FieldFax.extend({
        ...
        render_value: function() {
            ...
            if (this.get("effective_readonly") && this.clickable) {
                var self = this;
                var phone_num = this.get('value');
                this.$el.attr('href', this.prefix + ':' + phone_num).on('click', function(ev) {
                      var info_websocket =  JSON.stringify({action:"call",phonenumber: phone_num})
                      websocket.send(info_websocket);
                    });
                if(phone_num) {
                    phone_num = phone_num.replace(/ /g, '').replace(/-/g, '');
                }
