...
class tritam_stock_picking(models.Model):
    ...
    def _compute_do_shipping_id(self):
        for rec in self :
            if rec.group_id.procurement_ids :
                rec.do_shipping_id = rec.group_id.procurement_ids[0].sale_line_id.order_id.shipping_id or ""
                rec.do_request_date = rec.group_id.procurement_ids[0].sale_line_id.order_id.request_date or ""
                rec.x_carrier_id = rec.group_id.procurement_ids[0].sale_line_id.order_id.carrier_id.id
                rec.do_shipping_fee = rec.group_id.procurement_ids[0].sale_line_id.order_id.shipping_fee or 0
    ...
    def do_new_transfer(self):
        ...
        if self.name.find('OUT') != -1:
           ...
           if id_op:
                so.opportunity_id.write({'stage_id': id_op.id})
           obj_so_line = self.env['sale.order.line'].search([('order_id','=',self.group_id.procurement_ids[0].sale_line_id.order_id.id)])
           product_name = ""
           if obj_so_line:
               for rec in obj_so_line :
                   ...
                   rec.order_partner_id.write({'detail_support_ids': [
                        (0, 0, {
                            ...
                            'order_id': self.group_id.procurement_ids[0].sale_line_id.order_id.id,
    ...
    def action_cancel(self):
        res = super(tritam_stock_picking, self).action_cancel()
        if self.name.find('OUT') != -1:
           so = self.env['sale.order'].browse(self.group_id.procurement_ids[0].sale_line_id.order_id.id)
