...
class tritam_res_partner(models.Model):
    ...
    date_sub = fields.Date(string=u'Ngày phân bổ')
    ...
    phone_invalid = fields.Boolean('Sai số điện thoại', readonly=True, default=False)
    ...
    def _check_phone_is_number(self):
        ...
        if self.phone:
            self.phone_invalid = is_number(self.phone)
        ...
        for r in self:
            ...
            if r.category_id:
                for cate in r.category_id:
                    if len(str) == 0:
                        str = unicode(cate.name)
                    else:
                        str = unicode(str) + unicode(', ') + unicode(cate.name)
                r.str_category = str
    ...
    def create(self, vals):
        ...
        if 'street' in vals and vals['street'] :
            adress =  ("<li>Địa chỉ: "+ vals['street'] + "</li>").encode('utf-8')
        if 'comment' in vals and vals['comment'] :
            comment =  ("<li>Ghi chú ban đầu: "+ vals['comment'] + "</li>").encode('utf-8')
        if 'phone' in vals and vals['phone']:
            phone = ("<li>Điện Thoại: "+ vals['phone'] + "</li>").encode('utf-8')
        if 'mobile' in vals and vals['mobile']:
            mobile = ("<li>Di Động: "+ vals['mobile'] + "</li>").encode('utf-8')
        if 'source_customer' in vals and vals['source_customer']:
            source_customer =  ("<li>Nguồn: "+self.source_customer.browse([int(vals['source_customer'])]).name_get()[0][1] + "</li>").encode('utf-8')
        if 'category_id' in vals and len(vals['category_id']) > 0:
                ...
                for r in obj_tag:
                    category_id += r.name +","
                category_id = ("<li>Tag bệnh : "+ category_id + "</li>").encode('utf-8')
        body_html_partner = "<div><ul>" \
                            ...
                            "</ul></div>".format(sale_man=self.env.user.name.encode('utf-8'), time=fields.Date.today(),
                                                 name=vals['name'].encode('utf-8'),
                                                 adress=adress,category_id=category_id.encode('utf-8'),source_customer=source_customer,phone=phone,comment=comment,mobile=mobile)
    ...
    def write(self, vals):
        ...
        for r in self:
            ...
            if 'name' in vals and vals['name']:
                name =("<li>Tên: "+r.name+" -->"+vals['name']+"</li>").encode('utf-8')
                flag = True
            if 'street' in vals and vals['street']:
                old_street = r.street or  ""
                adress =("<li>Địa chỉ: "+old_street+" -->"+vals['street']+"</li>").encode('utf-8')
                flag = True
            if 'phone' in vals and vals['phone']:
                old_phone = r.phone or ""
                phone =("<li>Điện Thoại: "+old_phone+" -->"+vals['phone']+"</li>").encode('utf-8')
                flag = True
            if 'mobile' in vals and vals['mobile']:
                old_mobile = r.mobile or ""
                mobile =("<li>Di Động: "+old_mobile+" -->"+vals['mobile']+"</li>").encode('utf-8')
                flag = True
            if 'comment' in vals and vals['comment']:
                old_comment = r.comment or ""
                comment =("<li>Ghi chú ban đầu: "+old_comment+" -->"+vals['comment']+"</li>").encode('utf-8')
                flag = True
            if 'active' in vals:
                old_active = str(r.active) or ""
                active =("<li>Kích Hoạt: "+old_active+" -->"+str(vals['active'])+"</li>").encode('utf-8')
                flag = True
            if 'source_customer' in vals and vals['source_customer']:
                source = r.source_customer.browse([int(vals['source_customer'])]).name_get()[0][1].encode(
                    'utf-8')
                if len(r.source_customer.ids) > 0 :
                    old_source = r.source_customer.name_get()[0][1].encode('utf-8')
                else:
                    old_source = ""
                source_customer = ("<li>Nguồn: "+old_source+" -->"+source+"</li>").encode('utf-8')
                flag = True
            if 'category_id' in vals and len(vals['category_id']) > 0:
                arr_cat = [x for x in vals['category_id'][0][2]]
                obj_tag = self.category_id.browse(arr_cat)
                ...
                if len(r.category_id.ids) > 0 :
                    ...
                    old_cat = ""
                category_id = ("<li>Tag bệnh: "+old_cat+" -->"+cat+"</li>").encode('utf-8')
                flag = True
            body_html_partner = "<div><ul>" \
                                "<li>{sale_man} : {time}</li>" \
                                "<li>Chỉnh sửa </li>" \
                                ...
                                "</ul></div>".format(sale_man=self.env.user.name.encode('utf-8'),
                                                     ...
                                                     name=name,
                                                     adress=adress, category_id=category_id.encode('utf-8'),active=active,
