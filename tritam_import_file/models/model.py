                    ...
                    for sp in stock_picking_ids :
                        if sp.amount_total == money:
                            ...
                            if sp.name.find('PICK') != -1 and sp.state and sp.state == "assigned":
                                sp.do_new_transfer()
                                sit = stock_immediate_transfer.search([('pick_id', '=', sp.id)])
                                sit[-1].process()
                            if sp.picking_type_id.id == self.env.ref(
                                ...
                                so = self.env['sale.order'].browse(
                                    sp.group_id.procurement_ids[0].sale_line_id.order_id.id)
                                if so:
                        ...
                        for sp in stock_picking_ids:
                            ...
                            if int(x_status_do) == 2:
                                ...
                                if sp.name.find('PICK') != -1 and sp.state and sp.state == "assigned":
                                    ...
                                    sit = stock_immediate_transfer.search([('pick_id', '=', sp.id)])
                                    sit[-1].process()
                                if sp.picking_type_id.id == self.env.ref('stock.picking_type_out').id and sp.state and sp.state == "waiting" :
                                    sp.force_assign()
                            # Đơn thành Công
                            if int(x_status_do) == 3:
                                ...
                                if sp.name.find('PICK') != -1 and sp.state and  sp.state == "assigned":
                                    ...
                                    sit = stock_immediate_transfer.search([('pick_id', '=', sp.id)])
                                    sit[-1].process()
                                if sp.picking_type_id.id == self.env.ref('stock.picking_type_out').id and sp.state and sp.state == "assigned":
                                    sp.do_new_transfer()
                                    sit = stock_immediate_transfer.search([('pick_id', '=', sp.id)])
                                    sit[-1].process()
                                    so = self.env['sale.order'].browse(sp.group_id.procurement_ids[0].sale_line_id.order_id.id)
                                    if name_so :
                                        name_so.write({'x_status_do': 3})
                            # Đơn hoàn
                            if int(x_status_do) == 4:
                                if sp.name.find('PICK') != -1 and sp.state and sp.state == "waiting":
                                    sp.force_assign()
                                if sp.name.find('PICK') != -1 and sp.state and  sp.state == "assigned":
                                    sp.do_new_transfer()
                                ...
                                if sp.picking_type_id.id == self.env.ref('stock.picking_type_out').id and sp.state and sp.state != "done" :
                                    sp.action_cancel()
                                if name_so:
                                ...
                                if sp.name.find('PICK') != -1 and sp.state and  sp.state == "done":
                                    ...
                                    for sp_new in stock_picking_ids_new:
                                        if sp_new.name.find('RETURN') != -1 and sp_new.state and sp_new.state == "assigned":
                                            sp_new.do_new_transfer()
                                            sit = stock_immediate_transfer.search([('pick_id', '=', sp_new.id)])
                                            sit[-1].process()
                                if sp.picking_type_id.id == self.env.ref('stock.picking_type_out').id and sp.state and sp.state != "done":
                                    sp.action_cancel()
