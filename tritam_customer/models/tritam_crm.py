...
class tritam_crm_lead(models.Model):
    ...
    day_log_activity = fields.Date('Ngày hoàn thành cuối cùng')
    ...
    def create(self, vals):
        ...
        body_html_partner = "<div><ul>" \
                            "<li>Tạo mới </li>" \
                            "{category_id}" \
                            "</ul></div>".format(category_id=category_id.encode('utf-8'))
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals:
            ...
            name =("<li>Trạng Thái Ticket: "+old_state+" --> "+stage_name+"</li>").encode('utf-8')
            body_html_partner = "<div><ul>" \
                                "<li>{sale_man} : {time}</li>" \
                                "<li>Chỉnh sửa</li>" \
                                "{name}" \
...
class CRMStage(models.Model):
    ...
    status_opp = fields.Selection([('-1','Contact mới'),
                                   ('0', 'Contact failed'),
                                   ('1', 'Contact đang chăm sóc'),
                                   ('2', 'Contact thành công'),
                                   ('3', 'Contact mua nhưng hết hàng')], string="Loại Opportunity")
...
class ActivityLog(models.TransientModel):
    ...
    def action_schedule(self):
        for log in self:
            ...
            if self.next_activity_id:
                next_activity = ("<li>Hoạt động tiếp theo: " + self.next_activity_id.name + "</li>").encode('utf-8')
            if self.date_action:
                date_action = ("<li>Ngày hoạt động tiếp theo: " + self.date_action + "</li>").encode('utf-8')
            if self.title_action:
                title_action = ("<li>Tóm Tắt: " + self.title_action + "</li>").encode('utf-8')
    ...
    def action_log(self):
        ...
        for log in self:
            ...
            self.lead_id.day_log_activity = (datetime.date.today() + timedelta(hours=7)).strftime('%Y-%m-%d')
        return True
